...
object Par {
  ...
  private case class Map2Future[A, B, C](
    ...
    def get: C = {
      if (result.isEmpty) result = Some(f(af.get, bf.get))
      result.get
    }
    ...
    def get(timeout: Long, units: TimeUnit): C = {
      if (result.isEmpty) {
        ...
        val b = bf.get(remainingNanos, TimeUnit.NANOSECONDS)
        result = Some(f(a, b))
      }
      result.get
    }
    ...
    // For simplicity, cancel is not supported.
    def cancel(evenIfRunning: Boolean): Boolean = false
  ...
  def sequenceSimple[A](as: List[Par[A]]): Par[List[A]] =
    as.foldRight(unit(List.empty[A])) { case (a, acc) =>
      map2(a, acc)(_ :: _)
    }
